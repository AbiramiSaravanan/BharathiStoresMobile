...
import {addToCart,minusCart} from '../../project11/CustomerRedux/actions/cartDisplayActions';
...
class ProdItem extends React.Component{
    ...
    handleAddChange =  (product) => {
        ...
        var sp = product.sellingprice*product.gstpercent;
      const  {_id,sellingprice,productname} = product
      this.props.addToCart({_id,sp,productname, quantity:this.state.quans+1});  
              ...
              <Body>
                <Text>{p.productname}</Text> 
                 ...
                 <View style={{flexDirection:'row'}}>
                    <Text style={{ textDecorationLine: 'line-through' }}>Rs.{p.originalprice*p.gstpercent}</Text>
                    <Text>Rs.{p.sellingprice*p.gstpercent}</Text>
